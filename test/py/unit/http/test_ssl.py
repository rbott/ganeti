...
# All rights reserved.
#
# Tests for SSL certificate verification in threaded HTTP server.
#
...
class TestSSLContextCreation(unittest.TestCase):
  ...
  def test_ssl_server_wraps_socket(self):
    """Test that server socket is an ssl.SSLSocket when SSL is enabled."""
    ssl_params = http.HttpSslParams(
      ssl_key_path=self.key_path,
      ssl_cert_path=self.cert_path
    )
    ...
    self.assertIsInstance(test_server.socket, ssl.SSLSocket)
