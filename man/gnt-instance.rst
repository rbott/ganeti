...
boot\_type
    ...
    Only an explicit ``gnt-instance modify -H boot_type=uefi`` (instance
    stopped) creates and seeds the firmware disk; an inherited cluster
    default never does. Switching away from ``uefi`` keeps the disk in
    place but inert; it can be removed explicitly later.

    **start**, **failover** and **move** refuse to run for an instance
    that resolves to ``uefi`` (instance setting, cluster default, or
    start-time override) but has no firmware disk; while stopped, run
    ``gnt-instance modify -H boot_type=uefi <instance>`` to create it.
...
ovmf\_code
    ...
    ``add`` or ``gnt-instance modify -H boot_type=uefi``). Changing them
    afterwards on an instance with an existing firmware disk has no effect
    until a future re-seed maintenance command exists. Pair ``ovmf_code``
    and ``ovmf_vars`` from the same OVMF build; Ganeti cannot verify this.
